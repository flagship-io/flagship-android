...
class EAIManager() : OnEAIEvents {
                        ...
                        if (!responseContent.isNullOrEmpty()) {
                            ...
                            if (!segment.isNullOrEmpty()) {
                                ...
                                Flagship.configManager.decisionManager?.sendTroubleshootingHit(
                                    TroubleShooting.Factory.EMOTIONS_AI_SCORE.build(
                                        visitor,
                                        response,
                                        segment
                                    )
    ...
    fun init() {
        ...
        val gestureListener = EAIGestureListener(windowVisibleDisplayFrame, this)
        activityGestureListener = gestureListener
        runCatching {
            CoroutineScope(Job() + Dispatchers.Main).launch {
                try {
                    activityGestureDetector = GestureDetector(Flagship.application, gestureListener)
                } catch (e: Exception) {
                    FlagshipLogManager.exception(FlagshipException(e))
                }
            }
    ...
    internal fun startEAICollect(visitorDelegate: VisitorDelegate, activity: Activity? = null): Deferred<Boolean> {
            ...
            try {
                ...
                if (!visitorDelegate.configManager.flagshipConfig.eaiCollectEnabled) {
                    ...
                    if (!visitorDelegate.eaiScored) {
                        ...
                        sendEAIPageViewEvent()
                        val isEAICollectHasNotTimeout = eaiCollectLatch?.await(
                            (EAI_COLLECT_SESSION_TIMEOUT).toLong(),
                            TimeUnit.MILLISECONDS
                        ) ?: false
                        if (eaiCollectLatch != null && (eaiCollectLatch?.isCancelled() == false) && isEAICollectHasNotTimeout) {
        ...
        return collectCoroutine ?: Flagship.coroutineScope().async { false }
    }
    ...
    private fun setEAILastEventTimestamp() {
        ...
        if (eaiCollectStartTimestamp > 0 && visitorId != null) {
            ...
            if ((diffFromStartTimestamp >= EAI_COLLECT_TIMER) &&
                ...
                Flagship.coroutineScope().launch { endCollect() }
            }
        }
    }

    fun sendEAIPageViewEvent() {
        visitorDelegate?.visitorId?.let { visitorId ->
            ...
            val page = Page("${activityNamePrefix}${activityName}")
                ...
                .withVisitorIds(visitorId, visitorDelegate?.anonymousId)
            ...
            Flagship.coroutineScope().launch {
                ...
                response?.let {
                    ...
                    if (response?.code in 200..299) {
                        ...
                        log(
                            message = FlagshipConstants.Debug.EAI_COLLECT_START_TIMESTAMP.format(
                                eaiCollectStartTimestamp.toString()
                            )
                        )
