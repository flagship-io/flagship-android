...
////            withBodyParam(KeyMap.CUSTOM_VISITOR_ID.key, Flagship.customVisitorId ?: "")
            if (Flagship.visitorId.isNotEmpty() && Flagship.anonymousId != null) {
                withBodyParam(KeyMap.CUSTOM_VISITOR_ID.key, Flagship.visitorId)
                withBodyParam(KeyMap.VISITOR_ID.key, Flagship.anonymousId!!)
...
//                withBodyParam(KeyMap.TIMESTAMP.key, System.currentTimeMillis())
        ...
        fun withoutParam(param : String): HitRequestBuilder {
            withoutBodyParam(param)
            return this
        }
     ...
     */
    open class Screen(location: String) : HitBuilder<Screen>() {

        init {
            withHitParam(KeyMap.TYPE, Type.SCREENVIEW)
            withHitParam(KeyMap.ORIGIN, location)
        }
    }

    /**
     * Deprecated : Use Screen Hit instead.
     */
    @Deprecated("Use Hit.Screen instead", replaceWith = ReplaceWith("Hit.Screen"))
    class Page(location: String) : Screen(location)
     ...
     * @param productSku specifies the item code or SKU (optional)
     *
     */
    class Item(transactionId: String, productName: String, productSku : String) : HitBuilder<Item>() {
        ...
        init {
            ...
            withHitParam(KeyMap.ITEM_CODE, productSku)
        }
...
//            withHitParam(KeyMap.CUSTOM_VISITOR_ID, Flagship.anonymousId)
        }
    }
    ...
    internal class  Batch(visitorId : String, anonymousId : String? = null,
...
//               //withHitParam(KeyMap.CUSTOM_VISITOR_ID, customVisitorId)
               if (visitorId.isNotEmpty() && anonymousId != null) {
                   withHitParam(KeyMap.CUSTOM_VISITOR_ID, visitorId)
                   withHitParam(KeyMap.VISITOR_ID, anonymousId)
               } else if (visitorId.isNotEmpty() && anonymousId == null) {
                   withHitParam(KeyMap.VISITOR_ID, visitorId)
               } else {
                   withHitParam(KeyMap.VISITOR_ID, anonymousId!!)
               }
        ...
        fun withChild(hits : List<HitData> = ArrayList()) : Batch {
            ...
            for (h in hits) {
                ...
                child.put(KeyMap.QUEUE_TIME.key, System.currentTimeMillis() - h.timestamp)
                batch.put(child)
            }
