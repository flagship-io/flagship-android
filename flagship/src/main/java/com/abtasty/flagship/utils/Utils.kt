...
//        }
        ...
        fun genVisitorId(context: Context, type : VisitorType = VisitorType.ANONYMOUS): String {

            val sharedPref = context.getSharedPreferences("_Flagship", Context.MODE_PRIVATE)
            var visitorId = sharedPref.getString(type.name + "_visitorId", "")
            if (visitorId == null || visitorId.isEmpty()) {
                ...
                val random = Random().nextInt(max - min + 1) + min
                visitorId = "${cal.get(Calendar.YEAR)}" +
                        cal.get(Calendar.MONTH).let {if (it < 10) "0"+(it+1) else ""+(it+1) } +
                        cal.get(Calendar.DAY_OF_MONTH).let { if (it < 10) "0$it" else "$it" } +
                        cal.get(Calendar.HOUR_OF_DAY).let { if (it < 10) "0$it" else "$it" } +
                        cal.get(Calendar.MINUTE).let { if (it < 10) "0$it" else "$it" } +
                        cal.get(Calendar.SECOND).let { if (it < 10) "0$it" else "$it" } +
                        "$random"
                val edit = sharedPref.edit()
                edit.putString(type.name + "_visitorId", visitorId)
                edit.apply()
            }
            return visitorId
        }
