...
internal class DatabaseManager {
    ...
    val MIGRATION_6_7 = object  : Migration(6, 7) {
        override fun migrate(database: SupportSQLiteDatabase) {
            database.execSQL("ALTER TABLE `hits` ADD COLUMN `anonymousId` TEXT default null")
        }
    }
        ...
        db?.let {
            if (hit.requestIds.isEmpty()) {
                val id = it.hitDao().insertHit(
                    HitData(
                        null, Flagship.clientId ?: "", Flagship.visitorId, Flagship.anonymousId, System.currentTimeMillis(),
    ...
    fun getNonSentHits(visitorId: String, anonymousId: String? = null, @IntRange(from = 0, to = 50) limit: Int = 50): List<HitData> {
        db?.let { return it.hitDao().getNonSentHits(Flagship.sessionStart, visitorId, limit) }
        return listOf()
    }

    fun getNonSentActivations(visitorId: String, anonymousId: String? = null, @IntRange(from = 0, to = 50) limit: Int = 50): List<HitData> {
        db?.let { return it.hitDao().getNonSentActivations(Flagship.sessionStart, visitorId, limit) }
        return listOf()
    }

    fun getVisitorsWithNonSentActivations() : List<HitData> {
        db?.let { return it.hitDao().getVisitorsWithNonSentActivations(Flagship.sessionStart) }
        return listOf()
    }

    fun getVisitorsWithNonSentHits() : List<HitData> {
        db?.let { return it.hitDao().getVisitorsWithNonSentHits(Flagship.sessionStart) }
        return listOf()
    }
